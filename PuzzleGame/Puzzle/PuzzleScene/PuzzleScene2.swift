...
class PuzzleScene2: SKScene {
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit2_row-\(position)"
        case .animal:
            return "police_car_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object2_row-\(position)" // Replace with your actual objects image prefix
        }
    }
        ...
        self.run(SKAction.wait(forDuration: 1)) {
            ...
            self.run(SKAction.wait(forDuration: 0.3)) {
                ...
                if self.type == .animal {
                    ...
                    self.voiceManager.speak(word: "Teddy Bear")
                } else {
                    self.voiceManager.speak(word: "Mango")
                }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit2_complete"
            case .animal: // New type added
                completionImage = "police_car_complete"
            case .objects:
                completionImage = "object2_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(21) {
