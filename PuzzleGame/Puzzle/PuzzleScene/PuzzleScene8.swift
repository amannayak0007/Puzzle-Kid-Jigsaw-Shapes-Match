...
class PuzzleScene8: SKScene {
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit8_row-\(position)"
        case .animal:
            return "cement_mixer_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object8_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Bulb")
        } else {
            self.voiceManager.speak(word: "Avocado")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit8_complete"
            case .animal: // New type added
                completionImage = "cement_mixer_complete"
            case .objects:
                completionImage = "object8_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(27) {
