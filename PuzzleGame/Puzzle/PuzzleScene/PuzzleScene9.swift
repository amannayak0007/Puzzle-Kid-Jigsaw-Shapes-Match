...
class PuzzleScene9: SKScene {
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit9_row-\(position)"
        case .animal:
            return "road_roller_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object9_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "aeroplane")
        } else {
            self.voiceManager.speak(word: "Lemon")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit9_complete"
            case .animal: // New type added
                completionImage = "road_roller_complete"
            case .objects:
                completionImage = "object9_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(28) {
