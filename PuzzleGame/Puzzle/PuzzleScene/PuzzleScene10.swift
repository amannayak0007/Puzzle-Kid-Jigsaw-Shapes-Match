...
class PuzzleScene10: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        piece3 = createNode(at: CGPoint(x: 518, y: 280),
                           imageNamed: getImageName(for: "1-col-1", type: type),
                           size: pieceSize,
                           myName: "piece3")

        piece4 = createNode(at: CGPoint(x: 518, y: 700),
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit10_row-\(position)"
        case .animal:
            return "crane_car_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object10_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Tree")
        } else {
            self.voiceManager.speak(word: "Pear")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit10_complete"
            case .animal: // New type added
                completionImage = "crane_car_complete"
            case .objects:
                completionImage = "object10_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(29) {
