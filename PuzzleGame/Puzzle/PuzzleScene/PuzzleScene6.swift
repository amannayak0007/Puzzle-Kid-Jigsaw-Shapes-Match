...
class PuzzleScene6: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        piece1 = createNode(at: CGPoint(x: 518, y: 280),
                           imageNamed: getImageName(for: "2-col-1", type: type),
                           size: pieceSize,
                           myName: "piece1")

        piece2 = createNode(at: CGPoint(x: 1918, y: 280),
        ...
        piece4 = createNode(at: CGPoint(x: 1918, y: 700),
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit6_row-\(position)"
        case .animal:
            return "bulldozer_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object6_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Bus")
        } else {
            self.voiceManager.speak(word: "banana")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit6_complete"
            case .animal: // New type added
                completionImage = "bulldozer_complete"
            case .objects:
                completionImage = "object6_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(25) {
