...
class PuzzleScene5: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        piece1 = createNode(at: CGPoint(x: 1918, y: 700),
                           imageNamed: getImageName(for: "2-col-1", type: type),
                           size: pieceSize,
                           myName: "piece1")

        piece2 = createNode(at: CGPoint(x: 518, y: 700),
                           imageNamed: getImageName(for: "2-col-2", type: type),
                           size: pieceSize,
                           myName: "piece2")

        piece3 = createNode(at: CGPoint(x: 1918, y: 280),
                           imageNamed: getImageName(for: "1-col-1", type: type),
                           size: pieceSize,
                           myName: "piece3")

        piece4 = createNode(at: CGPoint(x: 518, y: 280),
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit5_row-\(position)"
        case .animal:
            return "garbage_truck_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object5_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Chair")
        } else {
            self.voiceManager.speak(word: "Grape")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit5_complete"
            case .animal: // New type added
                completionImage = "garbage_truck_complete"
            case .objects:
                completionImage = "object5_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(24) {
