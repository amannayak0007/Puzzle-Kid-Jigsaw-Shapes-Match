...
class PuzzleScene4: SKScene {
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit4_row-\(position)"
        case .animal:
            return "fire_truck_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object4_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Shoes")
        } else {
            self.voiceManager.speak(word: "Pineapple")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit4_complete"
            case .animal: // New type added
                completionImage = "fire_truck_complete"
            case .objects:
                completionImage = "object4_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(23) {
