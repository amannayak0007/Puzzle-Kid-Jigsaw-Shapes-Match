...
class PuzzleScene3: SKScene {
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit3_row-\(position)"
        case .animal:
            return "ambulance_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object3_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Basket ball")
        } else {
            self.voiceManager.speak(word: "Orange")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit3_complete"
            case .animal: // New type added
                completionImage = "ambulance_complete"
            case .objects:
                completionImage = "object3_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(22) {
