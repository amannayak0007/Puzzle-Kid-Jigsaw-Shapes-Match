...
class PuzzleScene7: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        piece1 = createNode(at: CGPoint(x: 1918, y: 280),
                           imageNamed: getImageName(for: "2-col-1", type: type),
                           size: pieceSize,
                           myName: "piece1")

        piece2 = createNode(at: CGPoint(x: 1918, y: 700),
                           imageNamed: getImageName(for: "2-col-2", type: type),
                           size: pieceSize,
                           myName: "piece2")

        piece3 = createNode(at: CGPoint(x: 518, y: 700),
    ...
    func getImageName(for position: String, type: PuzzleType) -> String {
        switch type {
        case .fruit:
            return "fruit7_row-\(position)"
        case .animal:
            return "excavator_row-\(position)" // Assuming this is your animal image
        case .objects:
            return "object7_row-\(position)" // Replace with your actual objects image prefix
        }
    }
    ...
    func createDoneScene(imageNamed name: String) {
        ...
        if self.type == .animal {
            ...
            self.voiceManager.speak(word: "Scissor")
        } else {
            self.voiceManager.speak(word: "Carrot")
        }
    ...
    fileprivate func checkPuzzleCompletion() {
        ...
        if (piece1isComplete && piece2isComplete && piece3isComplete && piece4isComplete) {
            ...
            switch type {
            case .fruit:
                completionImage = "fruit7_complete"
            case .animal: // New type added
                completionImage = "excavator_complete"
            case .objects:
                completionImage = "object7_complete"
            }
    ...
    func removeSuccess() {
        if let viewLoading = self.view?.viewWithTag(26) {
