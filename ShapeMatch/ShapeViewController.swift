...
class ShapeViewController: UIViewController {
    ...
    func setConstraints() {
        ...
        self.greatJobImg.heightAnchor.constraint(equalToConstant: 250).isActive = true
        self.greatJobImg.widthAnchor.constraint(equalToConstant: 250).isActive = true
    }
    ...
    func checkCollisions() {
        ...
        if berryImg.frame.intersects(berryShadowImg.frame) {
            ...
            if isSoundPlayed4 {
                ...
                berryImg.animateLabel1()
                spellShape(name: "Pentagon")
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
                    self.snowEmitterLayer.removeFromSuperlayer()
                }
            }
